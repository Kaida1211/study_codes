...
volatile unsigned long tc_sec;
volatile unsigned long tc_min;
volatile unsigned long tc_hour;
...
void init_MODE_DISPLAY(char mode)
{
	if(mode == 1)
	{
		lcd_xy(1,1);
...
void init_MODE()
{
	if(pre_mode != mode)
	{
		...
		lcd_clear();
		init_MODE_DISPLAY(mode);
		time_10m_count = 0;   //チャタリング除去後の初期化
		flush_lcd();
	}
}

void STOP_WATCH(unsigned long time)
{
	tc_sec = time_10m_count / 100;
	tc_min = tc_sec / 60;
	tc_hour = tc_min / 60;

	lcd_xy(1,2);
	lcd_dataout((tc_hour/10)%6);
	lcd_dataout(tc_hour%10);
	lcd_put(':');
	lcd_dataout((tc_min/10)%6);
	lcd_dataout(tc_min%10);
	lcd_put(':');
	lcd_dataout((tc_sec/10)%6);
	lcd_dataout(tc_sec%10);
	lcd_put(':');
	lcd_dataout((time_10m_count/10)%6);
	lcd_dataout(time_10m_count%10);
}
		...
		switch(mode)
		{
			...
			case 2:
				...
				if(PORTH.PIDR.BIT.B3 == 0)
				{
					...
					sound++;
					if(sound > 3) sound = 0;
					if(sound == 1) MELODY = GAKUHU_LENGTH;
					if(sound == 2)	MELODY = GAKUHU_LENGTH;
					{
						MELODY = 0;
						MELODY = GAKUHU_LENGTH2;
					}

					while(MELODY)
					{
						beep(GAKUHU[GAKUHU_LENGTH-MELODY][0],GAKUHU[GAKUHU_LENGTH-MELODY][1]);
