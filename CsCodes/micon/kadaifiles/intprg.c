...
extern unsigned long time_10m_count;
extern unsigned long btime;
extern unsigned int roi;

extern unsigned char sw_case1;
extern unsigned char sw_case2;
extern unsigned char sw_case3;
extern unsigned char sw_case4;
...
extern unsigned int GAKUHU;
extern unsigned int GAKUHU2;

/*
extern unsigned long tc_sec;
extern unsigned long tc_min;
extern unsigned long tc_hour;
*/
...
void Excep_CMT0_CMI0(void)
{
	...
	if(sw_case1 == 1)  STOP_WATCH(time_10m_count);

	if(sw_case3 == 1)  MEAS_TEMP();

	if(sw_case4 == 1)
	{

		if((time_10m_count % 100) == 0)
		{
			roi--;

			//min
			lcd_xy(1,2);
			lcd_dataout((roi/6000)%10);
...
//			time_10m_count = 0;
			...
			{
				sw_case4 = 0;
				if(roi == 0) beep(DO1,500);
			}
		}
	}

	else roi = MTU1.TCNT / 4 ; //roi自体はMTU1で別
...
void Excep_CMT1_CMI1(void)
{
	...
	if(PORTH.PIDR.BIT.B0 == 0) btime = 0;
...
void Excep_ICU_IRQ0(void)
{
		...
		STOP_WATCH(time_10m_count);

		flush_lcd();

		if(PORTH.PIDR.BIT.B1 == 0)
		{
			sound++;
			if(sound == 1)	MELODY = GAKUHU_LENGTH2;
		}
...
{
	if(sw_case1 == 1) sw_case1 = 0;

	if(sw_case2 == 1) btime = 0;
}
